...
from ..theme import (
    ...
    TASK_TYPE_COLORS, TASK_MODE_COLORS, INPUT_PROPS,
)
...
def _render_task_row(task: dict, render_fn):
    ...
    with ui.card().classes(card_cls).props("flat bordered") as card:
        view_row = ui.column().classes("w-full gap-2")
        with view_row:
            with ui.row().classes("w-full items-center gap-2 flex-nowrap"):
                with ui.row().classes("items-center gap-1 flex-wrap min-w-0 flex-grow"):
                    ui.badge(
                    ...
                    ui.label(f"#{tid}").classes("text-xs text-gray-500")
                ...
                with ui.row().classes("items-center gap-0 shrink-0"):
                    ...
                    ui.switch(value=enabled, on_change=on_toggle).props(SWITCH)
        ...
        with edit_form:
            ...
            with ui.row().classes("w-full gap-2 flex-wrap items-end"):
                type_sel = ui.select(
                    ...
                    value=ttype,
                ).props(INPUT_PROPS).classes("min-w-[120px] flex-1")
